...
const TabNavigation = ({ activeTab, onTabChange }: { activeTab: string; onTabChange: (tab: string) => void }) => {
  const tabs = [
    { id: 'vendeuse', label: 'Vendeuse', icon: User, color: '#477A0C' },
    { id: 'produits', label: 'Produits', icon: Package, color: '#C4D144' },
    { id: 'reglements', label: 'Règlements', icon: CreditCard, color: '#89BBFE' },
    { id: 'diverses', label: 'Diverses', icon: ShoppingCart, color: '#D68FD6' },
    { id: 'annulation', label: 'Annulation', icon: RotateCcw, color: '#FF6B6B' },
    { id: 'ca', label: 'C.A.', icon: BarChart3, color: '#4ECDC4' },
  ];
...
const ProductGrid = ({ products, onAddToCart }: { 
  ...
  const categories = ['all', 'Matelas', 'Accessoires', 'Sommiers', 'Oreillers', 'Divers'];
...
const CartSummary = ({ cart, onUpdateQuantity, onRemoveItem, onProceedToPayment }: {
  cart: CartItem[];
...
const PaymentModal = ({ isOpen, cart, total, onPayment, onClose }: {
  isOpen: boolean;
  cart: CartItem[];
  total: number;
  ...
  const paymentMethods = [
    ...
    { id: 'card', label: 'Carte', color: '#2196F3' },
    { id: 'check', label: 'Chèque', color: '#FF9800' },
    { id: 'multi', label: 'Mixte', color: '#9C27B0' },
  ];
...
// Main App Component
const App = () => {
  const [activeTab, setActiveTab] = useState('vendeuse');
  ...
  useEffect(() => {
    const savedCart = loadFromStorage('myconfort-cart', []);
    const savedSales = loadFromStorage('myconfort-sales', []);
    const savedVendor = loadFromStorage('myconfort-current-vendor', null);
  ...
  useEffect(() => {
    saveToStorage('myconfort-cart', cart);
  }, [cart]);

  useEffect(() => {
    saveToStorage('myconfort-sales', sales);
  }, [sales]);

  useEffect(() => {
    saveToStorage('myconfort-current-vendor', selectedVendor);
  }, [selectedVendor]);
    ...
    switch (activeTab) {
      ...
      case 'ca':
        const totalSales = sales
          .filter((sale: Sale) => !sale.canceled)
